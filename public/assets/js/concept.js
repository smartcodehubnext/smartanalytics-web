$(document).ready(function() {
  ...
  var compactMQL = window.matchMedia(
      ...
      $(".chat-write form input").on("keypress", function(e) {
        if (e.which == 13 && !$(this).val().length == 0) {
          if (
            ...
            $(
              '<span className="chat-bubble-text">' + $(this).val() + "</span>"
            ).insertAfter(
              ".right-sidebar-chat .chat-bubbles .chat-bubble:last-child span:last-child"
            );
          } else {
            $(
              '<div className="chat-bubble me"><div className="chat-bubble-text-container"><span className="chat-bubble-text">' +
                $(this).val() +
                "</span></div></div>"
            ).insertAfter(
              ".right-sidebar-chat .chat-bubbles .chat-bubble:last-child"
